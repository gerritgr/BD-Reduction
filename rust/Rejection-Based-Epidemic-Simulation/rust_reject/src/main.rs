...
fn read_arguments() -> (String, String, f64) {
    let args: Vec<String> = env::args().collect();
    return (args[1].clone(), args[2].clone(), args[3].clone().parse().unwrap());
}
...
fn perform_step(
    ...
    if was_successful && (current_event.new_target_state == State::Infected)  {
        ...
        if current_counts.infected_count == 0{
            current_counts.susceptible_count -= 1;
            current_counts.infected_count += 1;
            node_2_nodeinfo[current_event.src_node].state  = State::Infected 
        }
        }
...
fn main() {
    ...
    let (graphpath, outpath, infection_rate) = read_arguments();
    ...
    while current_time < HORIZON {
        ...
        if current_step % 10000000 == 0 {
            print!(".");
            if current_step % 1000000000 == 0 {
                println!("\ntime: {}", current_time);
            }
        }
    }
